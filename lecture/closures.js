...
function container10() {
  ...
  const q = x();
  // What data type is q? How could you know, based on the line below? function!
  const n = q();
  // console.log("n", n);
}
container10();
...
function container40() {
  ...
  for (var i = 1; i <= 5; i++) {
    const val = i;
    setTimeout(() => console.log(`timeout for i = ${val}`), val * 1000);
    // inc(i);
  }
}
// container40();
...
// yes its using something outside of itself
...
class School {
  ...
  addStudent(studentId, name) {
    this.students.push({ studentId: studentId, name: name });
    // this.students[studentId] = name;
  }
  deleteStudent(studentId) {
    let index = this.students.findIndex(student => student.studentId === studentId);
    this.students.splice(index, 1);
    // delete this.students[studentId];
  }
}

const test = new School();
test.addStudent(1, "madi");
test.addStudent(2, "christian");
